...
\textbf{Design contribution.} This gripper applies the same mode-switching logic through a compound \texttt{<fixed>} tendon in MuJoCo, replacing the gear differential with a purely tendon-based implementation. Spring-loaded distal phalanges add a third passive DOF for fingertip shape adaptation. The MJCF file is the functional CAD model---bodies, joints, dimensions, and constraints defined once, exercised in simulation. Three passive DOFs serving \emph{qualitatively different} functions (wrist rotation, right- and left-finger conformation) from one position command, no sensors.
...
\end{equation}
The stiffness $k_w$ is chosen to be large relative to the zero-load finger torque, strongly biasing the mechanism toward finger closure in the initial part of the motion. However, in contact, the wrist can still deflect slightly before a fully sharp phase boundary is reached. % The spring is small enough so that elevated post-stall actuator force drives clear wrist-dominated motion later in the actuation.
...
The differential works by contrast to stiffness. Fingers do not have intrinsic spring resistance, so tendon shortening initially goes \emph{predominantly} into $\theta_{R,L}$, while the wrist spring ($k_w = 5.0$\,N$\cdot$m/rad) biases the mechanism against rotation. In the ideal no-contact limit this yields a clean two-stage response; in contact, a small amount of wrist compliance can appear before a perfectly sharp boundary is reached. Once the fingers stall, either on object contact or at the joint limit, continued actuator force is redirected primarily into the wrist. The transition is governed entirely by torque balance: \emph{no control-law change, no sensor, no external trigger}.
...
Fig.~\ref{fig:phase} is the central result. $\ell_\text{ctrl}$ ramps $0\to0.8$ over 2\,s (Phase~1), then $0.8\to2.2$ over 3\,s (Phase~2), held 1\,s, reversed. Object: cylinder, radius 10\,mm.
...
\textbf{Phase portrait:} Strongly anisotropic, with a rounded corner consistent with compliant blending---\emph{approximately sequential}, not an ideal L.
...
Fig.~\ref{fig:screenshots} confirms all three behaviours visually. The red arrow on the wrist disc indicates rotation; its displacement in panel~(d) shows the wrist angle produced by Phase~2.
...
A 1-DoF underactuated gripper extracts three passive mechanical functions---wrist rotation, right and left distal conformation---from one position-controlled actuator, with no sensors, no switching logic, and no hardware beyond a compound tendon and two springs. The mechanism self-selects which subsystem moves based on instantaneous load: the wrist coefficient (0.25) and spring stiffness ($k_w=5.0$\,N$\cdot$m/rad) bias the motion toward finger closure first, followed by wrist-dominated rotation after contact-limited stall. In practice the handoff is blended rather than perfectly sharp, but no control intelligence is required.

MuJoCo~3.3.3 simulation confirmed: (1)~approximately sequential non-coplanar actuation from a monotone input (Fig.~\ref{fig:phase}); (2)~geometry-adaptive distal deflection across cylinder, sphere, and box (Fig.~\ref{fig:multishape}); (3)~reliable grasping over \textbf{8--36\,mm} diameter (Fig.~\ref{fig:sweep}).
