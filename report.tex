...
\textbf{Design contribution.} This gripper applies the same mode-switching logic through a compound \texttt{<fixed>} tendon in MuJoCo, replacing the gear differential with a purely tendon-based implementation. Spring-loaded distal phalanges add a third passive DOF for fingertip shape adaptation. The MJCF file is the functional CAD model---bodies, joints, dimensions, and constraints defined once, exercised in simulation. The result is a mechanism with three passive DOFs serving \emph{qualitatively different} functions (wrist rotation, right- and left-finger conformation) from one position command, with no sensors.
