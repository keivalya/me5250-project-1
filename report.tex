...
The differential works by contrast to stiffness. Fingers do not have intrinsic spring resistance, so tendon shortening initially goes completely into $\theta_{R,L}$. The wrist spring ($k_w = 5.0$\,N$\cdot$m/rad) resists rotation until the actuator force is high enough to overcome it---which only happens once the fingers stall, either on object contact or at the joint limit. At that point, the wrist begins to rotate. The transition is governed entirely by torque balance: \emph{no control-law change, no sensor, no external trigger}.
...
\begin{figure}[t]
  ...
  \caption{Phase-transition analysis, cylinder (radius 10\,mm).
    ...
    \textit{P4}: \textbf{L-shaped} phase portrait (finger vs.\ wrist) --- two non-coplanar motions, one input, no sensors.}
  \label{fig:phase}
\end{figure}
